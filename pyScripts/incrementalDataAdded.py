...
sourceFolder2= "C:/Users/Rongji He/Desktop/incremental_data/"
fileList= os.listdir(sourceFolder)

fileList = [x for x in fileList if "FinalVersion.csv" in x]

for file in fileList:

    df  = pd.read_csv(sourceFolder  + file,                                                   encoding = "ISO-8859-1")
    df1 = pd.read_csv(sourceFolder2 + file[:-16] + "/" + file[:-16] + "_accesses.csv",        encoding = "ISO-8859-1")
    df2 = pd.read_csv(sourceFolder2 + file[:-16] + "/" + file[:-16] + "_calls.csv",           encoding = "ISO-8859-1")
    df3 = pd.read_csv(sourceFolder2 + file[:-16] + "/" + file[:-16] + "_file_inclusions.csv", encoding = "ISO-8859-1")
    df4 = pd.read_csv(sourceFolder2 + file[:-16] + "/" + file[:-16] + "_sets.csv",            encoding = "ISO-8859-1")
    ...
    df.at[i,'distinct_authors_to_now']  = df.at[i,'distinct_authors_to_now'] +1.02-1.02
    df.to_csv(sourceFolder  + file[:-4] + "2.csv",index=False,encoding="ISO-8859-1")
