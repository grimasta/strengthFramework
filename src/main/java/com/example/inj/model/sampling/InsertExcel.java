...
//        sampleVectors.entrySet().forEach(e->System.out.println(e));
        ...
        for (String key : sampleVectors.keySet()) {
            ...
            for (List<Object> objLis : vectorList) {
               ...
               if((Boolean)objLis.get(6))
                ...
                //Hard Coded the Slope value when the commit in the segment is greater then 2   || (int)objLis.get(2) > 2
                if (objLis.get(2).toString().equalsIgnoreCase("U")) {
                    slope = -1;
                } else {
                    slope = 1;
                }
                //row.createCell(7).setCellValue(objLis.get(2).toString()); // number of commit in the segment
    ...
    /*
    This method will return the data associated with overallStrength into the excel.
    ...
    public void insertOverallStrength() {
        ...
        Sheet sheet = workbook.createSheet(ProjectNameContainer.PROJECT_NAME);
            ...
            for (String key : strength.keySet()) {
                ...
                subStrength=subStrength.entrySet()
                        .stream()
                        .map(entry -> {
