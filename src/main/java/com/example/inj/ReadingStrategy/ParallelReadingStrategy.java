...
public class ParallelReadingStrategy implements IReadingStrategy {
    ...
    public ParallelReadingStrategy() {
    ...
    public void parseData() throws IOException, ParseException {
        ...
        BeanListProcessor<IncrementalField> rowProcessor2 = new BeanListProcessor<>(IncrementalField.class);
        ...
        parserSettings2.setHeaders("Commit_id", "Accesses", "Calls","Inclusions", "Sets");

        // Select Attributes from enum
        parserSettings.selectFields(SelectAttributes.values());
        parserSettings2.selectFields("Commit_id", "Accesses", "Calls","Inclusions", "Sets");

        CsvParser parser = new CsvParser(parserSettings);
        CsvParser parser2 = new CsvParser(parserSettings2);
...
//          TODO clean up the TryFileDetails class and calls, its a huge mess
            TryFileDetails.TryFileDetailsBuilder tfdBuilder = new TryFileDetails.TryFileDetailsBuilder(af.getFile_id(), af.getId());
...
//		commitDetails2TryFileDetailsMapIterator = null;
    ...
    private Map<String, List<String>> makeFileIds2CommitIdsMap(
        ...
        for (Map.Entry<CommitDetails, List<TryFileDetails>> commitDetails2TryFileDetailsListEntry : Commits2FileChangesMap
                .entrySet())
...
//		subOut is a list from commitDates to CommitIsBugFixing values
        ...
        for (String row : readableMappingPairMap.keySet()) {
            for (String col : readableMappingPairMap.get(row).keySet()) {
                for (Map.Entry<Integer, List<Object>> file2FileEntry : readableMappingPairMap.get(row).get(col).entrySet()) {
            ...
            if (readableMappingSame.contains(row, row)) {
                for (Map.Entry<Integer, List<Object>> file2SelfEntry : readableMappingSame.get(row, row).entrySet()) {
